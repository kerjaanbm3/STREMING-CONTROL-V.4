...
const defaultStats: StatItem[] = [
  { label: 'Shots', leftValue: 7, rightValue: 0 },
  { label: 'Shots on Target', leftValue: 6, rightValue: 0 },
  { label: 'Possession %', leftValue: 58, rightValue: 42 },
  { label: 'Tackles', leftValue: 7, rightValue: 14 },
  { label: 'Fouls', leftValue: 1, rightValue: 3 },
  { label: 'Corners', leftValue: 0, rightValue: 1 },
  { label: 'Shot Accuracy %', leftValue: 87, rightValue: 0 },
  { label: 'Pass Accuracy %', leftValue: 92, rightValue: 81 },
];

export default function SoccerScoreboard() {
  // Mengambil state realtime dari controller Admin
  ...
  // Menghubungkan data dari state ke variabel yang akan ditampilkan
  ...
  // Format timer dari detik ke format MM:SS
  ...
  return (
    // PENTING: bg-transparent sangat penting agar latar belakang tembus pandang di OBS
        ...
        <div className="w-[96%] bg-gradient-to-b from-slate-800 via-indigo-900 to-blue-950 border-[4px] border-slate-300 rounded-[24px] -mt-5 pt-8 pb-5 px-6 text-white shadow-2xl">
          ...
          <div className="flex justify-between items-center gap-4">
            ...
            <div className="w-24 h-28 rounded-xl border-2 border-white/80 shadow-lg flex flex-col justify-center items-center bg-[linear-gradient(90deg,#009246_33%,#ffffff_33%,#ffffff_66%,#ce2b37_66%)]">
              {state.teamA.logoUrl ? (
                <img src={state.teamA.logoUrl} alt="Logo" className="w-11 h-11 object-contain rounded-full border-2 border-black bg-white shadow" />
              ) : (
              ...
              <span className="text-[10px] font-black text-black bg-white/90 px-1.5 py-0.5 rounded mt-1.5 shadow truncate w-20 text-center uppercase">
                {state.teamA.institution || 'HOME'}
              </span>
            </div>
            ...
            <div className="w-24 h-28 rounded-xl border-2 border-white/80 shadow-lg flex flex-col justify-center items-center bg-rose-600">
              {state.teamB.logoUrl ? (
                <img src={state.teamB.logoUrl} alt="Logo" className="w-11 h-11 object-contain rounded-full border-2 border-black bg-white shadow" />
              ) : (
                <div className="w-11 h-11 rounded-full border-2 border-black bg-radial from-white to-gray-400 flex justify-center items-center font-bold text-xl shadow">
                  ⚽
                </div>
              )}
              <span className="text-[10px] font-black text-red-600 bg-white/90 px-1.5 py-0.5 rounded mt-1.5 shadow truncate w-20 text-center uppercase">
                {state.teamB.institution || 'AWAY'}
              </span>
            </div>
